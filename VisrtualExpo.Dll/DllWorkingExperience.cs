...
{
    public class DllWorkingExperience
    {
        /// <summary>
        ...
        public WorkExperience GetByPK(int Id)
        {
            ...
            {
                return entities.WorkExperience.FirstOrDefault(p => p.Id == Id);
            }
        }
        ...
        public WorkExperience GetByAttendeeId(int Id)
        {
            using (var entities = new ApplicationDbContext())
            {
                return entities.WorkExperience.FirstOrDefault(p => p.Attendee_User_Id == Id);
            }
        }
        ...
        public int Insert(WorkExperience Exhibition)
        {
            using (var entities = new ApplicationDbContext())
            {
                entities.WorkExperience.Add(Exhibition);
        ...
        public void Update(WorkExperience Exhibition)
        {
            using (var entities = new ApplicationDbContext())
            {
                WorkExperience dbExhibition = entities.WorkExperience.SingleOrDefault(p => p.Id == Exhibition.Id);
        ...
        public List<WorkExperience> GetAttendeeAllWorkExperience(int id)
        {
            using (var entities = new ApplicationDbContext())
            {
                try
                {
                    return entities.WorkExperience.Where(p=>p.Attendee_User_Id==id).ToList();
                }
                catch (Exception ex)
                {
                    // Log Exception
        ...
        public Boolean Delete(int Id)
        {
            using (var entities = new ApplicationDbContext())
            {
                WorkExperience dbExhibition = entities.WorkExperience.SingleOrDefault(p => p.Id == Id);

                if (dbExhibition == null)
                    return false;

                entities.WorkExperience.Remove(dbExhibition);
        ...
        public List<WorkExperience> Search(RequestAdminFilter filters)
        {
            ...
            using (var entities = new ApplicationDbContext())
            {
                var query = from RequestOrganizerFilter in entities.WorkExperience
        ...
        public int GetSearchCount(RequestAdminFilter filters)
        {
            using (var entities = new ApplicationDbContext())
            {
                var query = from Exhibition in entities.WorkExperience
                            select Exhibition;
