...
function ChangeStatus(NewStatus, id) {
    ...
    $.post("/api/ExhibitionApi/ChangeStatus", User, ChangeStatusCallback);
}
...
function LoadExhibitionWithCount() {
    if (IsHTML5) {
        sessionStorage["CustomerFilters"] = JSON.stringify(filters);
    }
    $("#loader").show();
    $.post("/api/ExhibitionApi/GetExhibitionWithCount", filters, LoadExhibitionWithCountCallBack);
}
function LoadExhibitionWithCountCallBack(data) {
