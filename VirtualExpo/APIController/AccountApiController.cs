...
namespace VirtualExpo.Web.APIController
{
    ...
    public class AccountApiController : Controller
    {
        ...
        [HttpPost]
        [Route("AttendeeLogin")]
        public async Task<ServiceResponse> AttendeeLogin([FromForm] LoginModel model)
        {
            string role = string.Empty; Int64 id = 0;
            try
            {
                ...
                {
                    result.IsSucceeded = false;
                    result.Message = "You Are not Authenticate";
                    return result;
                }
                string message = "";
                id = dbUser.Id;

                if (dbUser.UserType == Convert.ToByte(UserRoleType.Attendee)) { message = "/Home/EventHome"; role = "Attendee"; }
                int user = Convert.ToInt32(User.Identity.Name);
                var claims = new List<Claim>
                    {
                        new Claim(ClaimTypes.GivenName, dbUser.FirstName),
